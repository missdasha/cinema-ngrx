...
export class SeatsSelectionPageComponent implements OnInit, OnDestroy {
  ...
  seatsTypes: { Standard: Seat, 'Love Seats': Seat, VIP: Seat } = {
    Standard: {
      class: 'seat_standart',
    ...
    VIP: {
      class: 'seat_vip',
      description: 'Hаскладывающееся кресло с подставкой для ног и выдвижным столиком.'
    }
  };
  filmId: string;
  seanceId: string;
  ...
  ngOnInit() {
    ...
    this.authorizationService.getIsUserAuthorized().pipe(takeUntil(this.notifier$))
      .subscribe(
        value => this.isUserAuthorized = value
      );
  }
  ...
  selectSeat(row: number, seat: number, type: string) {
    if (!this.seance.occupiedSeats[row][seat]) {
      if (!this.isUserAuthorized) {
        alert('Для бронирования мест необходимо авторизоваться');
      }
      else {
